...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  ChatBloc() : super(const ChatInitial()) {
    ...
    on<ChatRecoverSession>(_onRecoverSession);
  }
  ...
  Future<void> _onInitialize(
    ...
    try {
      debugPrint(
        '💬 [CHAT BLOC] Inicializando chat - ClassId: ${event.classId}',
      );
  ...
  Future<void> _onLoadMessages(
    ...
    try {
      ...
      for (int i = 0; i < messages.length; i++) {
        debugPrint(
          '💬 [CHAT BLOC] Mensagem $i: ${messages[i].messageText} - ${messages[i].sentAt}',
        );
      }
      _messages = messages;
  ...
  Future<void> _onSendMessage(
    ...
    try {
      ...
      debugPrint(
        '💬 [CHAT BLOC] Mensagem marcada como pendente: ${event.messageText}',
      );
      ...
      if (!alreadyExists) {
        ...
        debugPrint(
          '💬 [CHAT BLOC] Mensagem adicionada: ${newMessage.messageText}',
        );
        debugPrint('💬 [CHAT BLOC] Mensagens depois: ${_messages.length}');
        debugPrint(
          '💬 [CHAT BLOC] Emitindo ChatLoaded com ${_messages.length} mensagens',
        );
        ...
        debugPrint(
          '💬 [CHAT BLOC] Mensagem já existe via WebSocket, não adicionando novamente',
        );
      }

      debugPrint(
        '💬 [CHAT BLOC] Mensagem enviada via API REST (WebSocket será emitido automaticamente pelo backend)',
      );
    } catch (e) {
  ...
  Future<void> _onConnectWebSocket(
    ...
    debugPrint(
      '💬 [CHAT BLOC] WebSocket conectado após connect(): ${_ws.isConnected}',
    );
    ...
    _wsSub = _ws.messageStream.listen((message) {
      debugPrint(
        '💬 [CHAT BLOC] Mensagem WebSocket recebida: ${message['type']}',
      );
    ...
    try {
      ...
      if (event.data['data'] is Map<String, dynamic>) {
        ...
        if (data['message'] is Map<String, dynamic>) {
          ...
          debugPrint(
            '💬 [CHAT BLOC] Usando estrutura event.data[\'data\'][\'message\'] (new_message)',
          );
        } else {
          // Fallback: data já é a mensagem (para message_sent/message_received)
          messageData = data;
          debugPrint(
            '💬 [CHAT BLOC] Usando estrutura event.data[\'data\'] diretamente',
          );
        }
      }
      ...
      if (messageData['receiverId'] == null) {
        ...
        debugPrint(
          '💬 [CHAT BLOC] receiverId corrigido para: ${messageData['receiverId']}',
        );
      }
      ...
      if (_classId != null && message.classId != _classId) {
        debugPrint(
          '💬 [CHAT BLOC] Mensagem de outra classe (${message.classId}), ignorando',
        );
        return;
      }

      // Verificar se é uma mensagem que acabamos de enviar (para evitar duplicação)
      final isMessageWeJustSent =
          _sentMessageTexts.contains(message.messageText) ||
          _pendingMessageIds.contains(message.id);

      if (isMessageWeJustSent) {
        debugPrint(
          '💬 [CHAT BLOC] Mensagem que acabamos de enviar via WebSocket, ignorando para evitar duplicação',
        );
        debugPrint(
          '💬 [CHAT BLOC] ID: ${message.id}, Texto: ${message.messageText}',
        );
        return;
      }
  ...
  Future<void> _onJoinClassRoom(
    ...
    try {
      ...
      _ws.emit('join_class', {'classId': _classId});
  ...
  Future<void> _onLeaveClassRoom(
    ...
    try {
      ...
      _ws.emit('leave_class', {'classId': _classId});
  ...
  Future<void> _onRecoverSession(
    ChatRecoverSession event,
    Emitter<ChatState> emit,
  ) async {
    if (_classId == null || _receiverId == null || _receiverName == null) {
      return;
    }

    try {
      debugPrint('💬 [CHAT BLOC] Recuperando sessão do chat...');
      ...
      debugPrint('❌ [CHAT BLOC] Erro ao recuperar sessão do chat: $e');
    }
  }
}
