...
class StudentLessonsHistoryPage extends StatefulWidget {
  const StudentLessonsHistoryPage({super.key});

  @override
  State<StudentLessonsHistoryPage> createState() => _StudentLessonsHistoryPageState();
}

class _StudentLessonsHistoryPageState extends State<StudentLessonsHistoryPage> {
  ...
  /// Avatar do personal (usa URL quando disponível, senão iniciais)
  Widget _buildPersonalAvatar(String personalName, {String? photoUrl}) {
    final initials = _getInitials(personalName);
    return ClipRRect(
  ...
  Widget _buildLessonCardFromClass(ClassResponseDto classData) {
    ...
    final trainerName = '${classData.personalFirstName ?? ''} ${classData.personalLastName ?? ''}'.trim();
    final location = classData.location;
    final price = classData.proposalPrice ?? 0.0;
    
    // Log para verificar valores da API
    print('🔍 [STUDENT_HISTORY] Aula ${classData.id}:');
    ...
    print('  - Status Pagamento: ${classData.paymentStatus}');
    
    final rating = classData.personalRating?.round() ?? 5; // Usar avaliação real do personal
          ...
          // Informações do treinador (personal)
                        Row(
                          ...
                          children: [
                            Container(
                              ...
                              child: _buildPersonalAvatar(
                                trainerName,
                                photoUrl: classData.personalProfileImageUrl,
                              ),
                            ),
                            const SizedBox(width: 12),
              Expanded(
