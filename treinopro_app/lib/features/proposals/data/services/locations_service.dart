...
class LocationsService {
  ...
  Future<List<TrainingLocation>> searchLocations(
    ...
    if (useCurrentLocation && (userLat == null || userLng == null)) {
      try {
        ...
        if (location != null) {
          ...
          print('📍 DEBUG: Usando localização atual: $finalUserLat, $finalUserLng');
        } else {
          print('📍 DEBUG: Localização não disponível - buscando sem coordenadas');
        }
      } catch (e) {
        print('⚠️ DEBUG: Erro ao obter localização, buscando sem coordenadas: $e');
      }
    }
    ...
    final uri = Uri.parse('$_baseUrl${ApiConstants.locationsSearch}').replace(
    ...
    if (response.statusCode == 200) {
      ...
      print('🔍 DEBUG: Erro na busca de locais: ${response.statusCode} - ${response.body}');
